...
$(document).ready(function () {
  $("#commandDiv").terminal(function (command, term) {
    if (/hungry|start|yes|y/i.test(command)) {
      term.push(function (command, term) {
        if (/1|Taiwan/i.test(command)) {
          currentRecipe = recipes.Taiwan;
        } else if (/2|CentralEurope/i.test(command)) {
          currentRecipe = recipes.CentralEurope;
        } else if (/3|Korea/i.test(command)) {
          currentRecipe = recipes.Korea;
        } else if (/4|India/i.test(command)) {
